...
df<-read.table(file="data2/heatmap/6-26dna/methyl-seq-18wk-heatmap.txt", sep ="\t", header = TRUE, check.names = FALSE)
#####heatmap needs numeric matrix. Only use the expression as matrix
df
df1 <- df[df$gene =="Tnf",]


df$gene1 <- c(rep("", 129), rep("Tnf",3), rep("",588))
...
heatmap.2(mat,col=gray.colors(255,1,0),dendrogram = "none",Colv = FALSE, 
...
dev.off()
##with key and labels
heatmap.2(mat,col=gray.colors(255,1,0),dendrogram = "none",Colv = FALSE,Rowv = FALSE,
          trace = "none",
...
#
heatmap.2(mat,col=gray.colors(255,0,1),Colv = FALSE,dendrogram = "none")# green(255))
dev.off()
#
#
