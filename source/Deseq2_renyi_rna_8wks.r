...
tab <- read.table("data/yue/comb.count", header = T, row.names = "Symbol")
tab <- tab[,-(2:7)] #remove columns 1 to 4. Other approaches may also work.
#Assign column names
colnames(tab) <- c("length", paste("RW",1:7, sep = ""))
colnames (tab) <- c("Length", paste("C", c(1,26,29,2,42,46,65,70,75,79), sep = ""))
# create sample matrix
mat <- matrix(paste("S",1:7, sep = ""), nrow = 7)
#
mat <- matrix(c("Control", "AOM+DSS", "AOM+DSS", "Control", "AOM+DSS+Cur.", "AOM+DSS+Cur.", "DSS", "DSS","DSS+Cur.", "DSS+Cur.", rep(0,3), "1", rep(0,6)), nrow = 10)
...
mat_f <- fpkm_72[!rowSums(fpkm_72 <1),]
fpkm_10_3 <- fpkm_10[!rowSums(fpkm_10 <1),]
#or
fpkm_10_2 <- fpkm_10[rowSums(fpkm_10 >= 1) >= 10,]
