...
boxplot(dt3,
        border = c("black"),
        at = c(1,3,5), # To make a gap between groups.This is optional.
...
dt4.1 <- melt.data.table(dt3,
                        variable.name = "Treatment",
                        value.name = "log10.meth.rat")
dt4.1$Treatment <- factor(dt4.1$Treatment,
                          levels = c("AOMDSS_18.mu",
                                     "Control_18.mu",
                                     "AOMDSSCur_18.mu"))

m1 <- lm(log10.meth.rat ~ Treatment,
         data = dt4.1)
m2 <- aov(log10.meth.rat ~ Treatment,
         data = dt4.1)
summary(m1)
summary(m2)
# > summary(m1)
# 
# Call:
#   lm(formula = log10.meth.rat ~ Treatment, data = dt4.1)
# 
# Residuals:
...
# Treatment        2     10   4.980   9.986 4.61e-05 ***
#   Residuals   138042  68842   0.499                     
# ---
#   Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1
