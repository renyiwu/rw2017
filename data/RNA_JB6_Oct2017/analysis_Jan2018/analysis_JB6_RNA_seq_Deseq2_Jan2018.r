...
#Renyi W. 2018-1-15
#JB6 cells treated with different compounds by Shan and Renyi
# S1. TPA_1 10 ng/mL, new batch RNA extracted by Renyi, 5-day treatment.
# S2. TPA_1 10 ng/mL + mITC 2.5 uM new batch RNA extracted by Renyi, 5-day treatment.
# S3. Control. old batch RNA extracted by Shan, 7-day treatment.
# S4. TPA 10 ng/mL. old batch RNA extracted by Shan, 7-day treatment.
# S5. TPA 10 ng/mL + CA 7.5 uM. old batch RNA extracted by Shan, 7-day treatment.
# S6. TPA 10 ng/mL + FX 6.25 uM. old batch RNA extracted by Shan, 7-day treatment.
# S7. TPA 10 ng/mL + CDDO 0.05 uM.old batch RNA extracted by Shan, 7-day treatment.
# S1 aka RW1 and the same for others.
...
# tab <- fread("data/RNA_JB6_Oct2017/RW_all_primary.dedup_hisat2_new.csv")
# tab <- data.frame(tab)
...
tab <- read.table("data/RNA_JB6_Oct2017/RW_all_primary.dedup_hisat2_new.csv", header = T, row.names = "Geneid")
...
tab_7 = tab[2:8] # or tab[-1]
# create sample matrix
mat <- matrix(c(paste("S",1:7, sep = ""), 1, 1, 0, 0, 0, 0, 0), nrow = 7, ncol = 2, byrow = F) #byrow default to F, meaning top-bottom then left-right
colnames(mat) <- c("condition", "batch")
rownames(mat) <- colnames(tab[2:8])
mat
mat_7 <- mat
...
dds <- dds[ rowSums(counts(dds) >= 5) >= 7, ]  # down to 11699 genes
#Run deseq
dds_7 <- DESeq(dds)
# pairwise comparisons
res1 <- results(dds_7 ,contrast=c("condition", "S7", "S1"))
#write out
write.table(res1, file="data/RNA_JB6_Oct2017/analysis_Jan2018/S7-S1.csv", sep="\t", quote=F, col.names=NA)
#
...
dds_7 <- DESeq(dds)
# pairwise comparisons
res1 <- results(dds_7 ,contrast=c("condition", "S3", "S4"))
#write out
write.table(res1, file="data/RNA_JB6_Oct2017/analysis_Jan2018/relevel-to-s4_s3-s4.csv", sep="\t", quote=F, col.names=NA)
#
#2
dds$condition <- relevel(dds$condition, ref="S1")
# filter for genes with at least one count in at least two samples:
...
res1 <- results(dds_7 ,contrast=c("condition", "S3", "S4"))
#write out
write.table(res1, file="data/RNA_JB6_Oct2017/analysis_Jan2018/relevel-to-s1_s3-s4.csv", sep="\t", quote=F, col.names=NA)
#
#
#
#
