...
library(edgeR)
dt <- read.table("data/uvb-skin/featurescounts-M_uvb-skin_dedup_renyi_2-8-2018.csv", header = T)
dt <- dt[-(2:5)]

#Define groups (and batches)
colnames(dt)[3:34] <- substring(colnames(dt),2,10)[3:34]
groups <- rep(1:16, each = 2)
timepoints <- c("02w", "15w", "25w", "25t")
treatments <- c("control", "UVB", "UVB+SFN", "UVB+UA")
...
y <- DGEList(counts = dt[3:34], group = groups, genes = dt[1:2])

#Define design

#  design <- model.matrix(~group+batch) # This style works only with "coef" setting as in glmLRT(fit, coef = 2)
# Or 
design <- model.matrix(~0+timepoints+treatments) # This style works only with "contrast" setting as in glmLRT(fit, contrast = c(0, -1, 1, 0, 0, 0))
...
write.table(rpkm,
            file = "data/uvb-skin/rpkm_all_uvb-skin_edgeR_2-9-2018.csv",
            sep = "\t", col.names = T, row.names = F) 
